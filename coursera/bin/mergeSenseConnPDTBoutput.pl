...
my %removed_files = ();
open( my $rem_fh, "<$path/../data/Removed_files_$dbname".".txt") 
		or die "\n Cannot open $path/../data/Removed_files_$dbname.txt";
...
foreach my $forum_id ( sort @$forums){
	...
	my $threads;
	opendir (my $dh, "$path/../$dbname"."_pdtbinput/$forum_id") 
			or die "Cannot open directory $path/../$dbname"."_pdtbinput/$forum_id";
	while(readdir $dh){
		if ( $_ =~ /^\.+/ || $_ =~ /output/){
			next;
		}		
